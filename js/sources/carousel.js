...
//GALERIA DINAMICA
export function galeriaDinamica(){
    var indexValue = 1;
    mostrarSlider(indexValue);
    const izquierda = document.getElementById('boton-1a');
    const derecha = document.getElementById('boton-2a');

    izquierda.addEventListener('click', () =>{
        mostrarSlider(indexValue += -1);
    })
    ...
    array.forEach((item, index) =>{
        item.addEventListener('click', (data) =>{
            mostrarSlider(indexValue = data.target.id.substr(-1));
        })
    })

    function mostrarSlider(foto){
        var i;
        const img = document.querySelectorAll('.slider-element');
        ...
        slider[indexValue-1].style.background='grey';
...
//GALERIA QUE CARGA DINAMICAMENTE
